...
You CANNOT delete triggers. If the user asks to delete, they must manage them manually.
...
5. **Validate connections** — Infer what services the agent will need from the user's description. For example, if they say "check Datadog and Slack me errors," the agent needs both Datadog and Slack MCP connectors. Cross-reference with the connectors list above. If any are missing, warn the user to configure them first.${gitRepoUrl ? ` The default git repo is already set to \`${gitRepoUrl}\`. Ask the user if this is the right repo or if they need a different one.` : ' Ask which git repos the remote agent needs cloned into its environment.'}
...
- To delete a trigger, users must manage them manually
      ...
      if (repo === null) {
        ...
        if (!hasAccess) {
          ...
          const msg = webSetupEnabled
            ? `GitHub not connected for ${repo.owner}/${repo.name} \u2014 run /web-setup to sync your GitHub credentials.`
            : `GitHub App not installed on ${repo.owner}/${repo.name} \u2014 configure GitHub access if your trigger needs this repo.`
          setupNotes.push(msg)
        }
      }
