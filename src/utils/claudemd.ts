  ...
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    for (const dir of dirs.reverse()) {
      ...
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        const projectPath = join(dir, 'OPENCLAUDE.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
            'Project',
        ...
        const dotClaudePath = join(dir, '.openclaude', 'OPENCLAUDE.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
            'Project',
      ...
      if (isSettingSourceEnabled('localSettings')) {
        const localPath = join(dir, 'OPENCLAUDE.local.md')
        result.push(
          ...(await processMemoryFile(
            localPath,
            'Local',
...
export function isMemoryFilePath(filePath: string): boolean {
  ...
  if (name === 'OPENCLAUDE.md' || name === 'OPENCLAUDE.local.md') {
    return true
  }
