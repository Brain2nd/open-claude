// MACRO global definition for development/unbundled mode
// When compiled via `bun build --define`, MACRO is injected at build time.
// For `bun run ./src/entrypoints/cli.tsx` (dev mode), provide defaults.
...
// OpenClaude defaults — all overridable via env vars.
// Set early so all downstream checks see the correct values.
...
if (!process.env.ANTHROPIC_MODEL) {
  process.env.ANTHROPIC_MODEL = 'z-ai/glm-5-turbo';
}
if (!process.env.CLAUDE_CONFIG_DIR) {
  process.env.CLAUDE_CONFIG_DIR = `${process.env.HOME}/.openclaude`;
}
