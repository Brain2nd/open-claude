...
export async function launchSSHDirectoryBrowser(root: Root, props: {
  conn: SSHConnectionManager;
}): Promise<string | null> {
  const { RemoteDirectoryBrowser } = await import('./components/ssh/RemoteDirectoryBrowser.js');
  return showSetupDialog<string | null>(root, done => <RemoteDirectoryBrowser conn={props.conn} initialPath="~" onSelect={done} onCancel={() => done(null)} />);
}
